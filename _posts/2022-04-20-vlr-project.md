...
Results for comparing the saliency maps generated by object detection models to the DeepGazeIIE's predicted eye-fixations on the MIT1003 dataset [**CITE**] are shown in Table 1. We calculated the mean absolute error (MAE) and intersection over union (IoU) for each model. The MAE (mean absolute error) is preferred to be close to 0.0; the IoU (intersection over union) is preferred to be close to 1.0. 
